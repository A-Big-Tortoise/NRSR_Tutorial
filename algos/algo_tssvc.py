...
def algo_tssvc(X, y, X_test, y_test, kernel='gak', gamma="auto"):
    clf = TimeSeriesSVC(kernel=kernel, gamma=gamma, probability=True)
    clf.fit(X, y)
...
if __name__ == '__main__':
    ...
    # Define a function to prepare data for TimeSeries SVC algorithm
    ...
    y_pred, score = algo_tssvc(X, y, X_test, y_test)

    # Compute the confusion matrix to evaluate the TimeSeries SVC classifier
    con_matrix = confusion_matrix(y_test, y_pred)

    # Display the results of the TimeSeries SVC algorithm
    print('Result of Knn')
    print('---accuracy---')
    print(score)
